...
'---------------------------------------------------------------
'- Registry Api Functions' Declarations...
'---------------------------------------------------------------
...
'---------------------------------------------------------------
' Registry Kinds...
Const REG_OPTION_NON_VOLATILE = 0     ' Keyword Reserved When System Restarts
' Registry Keywords Safety...
...
Public Enum ValueType
  REG_SZ = 1                 ' String
  REG_EXPAND_SZ = 2             ' String Extra
  REG_BINARY = 3               ' 0-1 Val
  REG_DWORD = 4               ' DWORD Val
  REG_MULTI_SZ = 7             ' String Multi
End Enum
...
Private hKey As Long             ' Opening Handle for Reg
Private i As Long, j As Long       ' Dw Variants
Private Success As Long           ' API val returned, get whether succeeded

'-------------------------------------------------------------------------------------------------------------
'- New Key words...
'- If ValueName and Value defaulted, only build KeyName vacant, with no sub key...
...
'-------------------------------------------------------------------------------------------------------------
Public Function GetKeyValue(KeyRoot As KeyRoot, KeyName As String, ValueName As String, Optional ValueType As ValueType = REG_SZ) As String
  ...
  Select Case ValueType                                     ' 通过判断关键字的类型, 进行处理
    Case REG_SZ, REG_MULTI_SZ, REG_EXPAND_SZ
        TempBin = StrConv(TempValue, vbFromUnicode)
        TempValue = StrConv(LeftB(TempBin, ValueSize - 1), vbUnicode)
...
'-------------------------------------------------------------------------------------------------------------
Public Function GetKeyInfo(ByRef KeyRoot As KeyRoot, ByRef KeyName As String, ByRef SubKeyName() As String, ByRef ValueName() As String, ByRef ValueType() As ValueType, Optional ByRef CountKey As Long, Optional ByRef CountValue As Long, Optional ByRef MaxLenKey As Long, Optional ByRef MaxLenValue As Long) As Boolean
  Dim f As FILETIME
